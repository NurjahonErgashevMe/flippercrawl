Firecrawl is a web scraper API. This workspace is trimmed to the API and `apps/nuq-postgres` for Docker Compose.
...
1. Write some end-to-end tests that assert your win conditions, if they don't already exist
  ...
  - These tests will be ran on a variety of configurations. You should gate tests in the following manner:
    ...
    - If it requires AI: `!process.env.TEST_SUITE_SELF_HOSTED || process.env.OPENAI_API_KEY || process.env.OLLAMA_BASE_URL || process.env.OPENROUTER_API_KEY`
